...
(()=>{
 ...
 function switchView(next){mode=next;const c=configs[mode==='plan'?'plan':angle];img.src='assets/'+c.src+'-original.png';img.alt=c.label;nav.setImage(...c.size);document.querySelector('#plan-mode-label').textContent=c.label;document.querySelector('#plan-angle').hidden=mode==='plan';document.querySelector('#plan-original').href='assets/'+c.src+'-original.png';pins.querySelectorAll('button').forEach((b,i)=>{b.style.left=c.pins[i][0]+'%';b.style.top=c.pins[i][1]+'%'});document.querySelectorAll('[data-plan-mode]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.planMode===mode));}
